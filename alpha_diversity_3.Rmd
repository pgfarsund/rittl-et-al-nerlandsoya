...
data.frame(
  ...
  mutate(effdiv = exp(Shannon)) -> df

### model effective number of species
plantmod <- lmerTest::lmer(effdiv ~ phase + (1|blokk), data = df)
summary(plantmod)
...
(df |>
  ...
  geom_boxplot(linewidth = 0.6, alpha = 0.5, outlier.shape = NA, width = 0.5) +
  geom_jitter(
...
(data.frame(
  ...
  mutate(effdiv = exp(Shannon), 
         sub_blokk = paste0(blokk, "_", sub_block)) -> df)

### model effective number of species
pitmod <- lmerTest::lmer(effdiv ~ phase + (1|blokk) + (1|sub_block), data = df)
summary(pitmod)
...
(data.frame(
  ...
  mutate(blokk = sapply(strsplit(Navn, split = '_', fixed = TRUE), `[`, 1), 
         effdiv = exp(Shannon)) -> df)

### model effective number of species
nemmod <- lmerTest::lmer(effdiv ~ phase + (1|blokk) + (1|month), data = df)
summary(nemmod)
...
### model effective number of species
funmod <- lmerTest::lmer(effdiv ~ phase + (1|blokk) + (1|month), data = df)
summary(funmod)
isSingular(funmod)

# the fungal model is a singular fit. this could symptom of our random effects having few levels or that the model is simply overfitted. from the output of summary(mod) we see that "month" explains no variance in the data. to get past this issue, we fit a Bayesian LMM with package blme and apply 

#apply a Gamma prior as recommended in Chung et al. 2013 (https://doi.org/10.1007/s11336-013-9328-2) and proposed by the authors of lme4 through help(isSingular)

library(blme)

funmodblme <- blme::blmer(effdiv ~ phase + (1|blokk) + (1|month), data = df, 
                          cov.prior = "gamma(5, 2.5)")
summary(funmodblme)
...
### model effective number of species
bacmod <- lmerTest::lmer(effdiv ~ phase + (1|blokk) + (1|month), data = df)
summary(bacmod)
...
```{r}
ggarrange(plantplot, inrich, pitrich, nemrich, funrich, bacrich, nrow = 2, ncol = 3)
```
