...
```{r}
here("")
plants <- data.frame(read.csv(file = here("../")))

plants <- data.frame(read.csv("data/plants/data_veg.csv", sep = ";")) %>% 
  t() %>% 
  data.frame() %>% 
  row_to_names(1) %>% 
  clean_names() %>% 
...
saveRDS(ps, "data/plants/heathland-fire-plants.RDS")
